...
public class MazeGenerator1 : MonoBehaviour
{
    ...
    void instantiateWallMaze()
    {
        for (int i = 0; i < size + 1; i++)
        {
            ...
            {
                hwalls[0, 0] = false;
                hwalls[size - 1, size] = false;
                if (hwalls[i, j] && i < size)
                {
                    var prefab = Instantiate(wall, new Vector3(i * scale + 450f, 15f, j * scale - scale / 2 + 50f), Quaternion.identity);
                    prefab.transform.localScale = new Vector3(scale, 3 * scale, scale / 4);
                    prefab.name = "hwall_{" + i.ToString() + "," + j.ToString() + "}";
                }
                if (vwalls[i, j] && j < size)
                {
                    var prefab = Instantiate(wall, new Vector3(i * scale - scale / 2 + 450f, 15f, j * scale + 50f), Quaternion.identity);
                    prefab.transform.localScale = new Vector3(scale / 4, 3 * scale, scale);
